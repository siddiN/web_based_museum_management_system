...
<head>
    ...
    <title>View Appraisals Records</title>
    <style>
        body {
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td>" . htmlspecialchars($row['appraisal_id']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['object_id']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['appraiser_name']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['appraisal_date']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['reason']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['description']) . "</td>";
                        echo "<td>
                                <a class='btn' href='update_appraisal_form.php?id=" . $row['appraisal_id'] . "'>Update</a>
                                <a class='btn btn-danger' href='delete_appraisal.php?id=" . $row['appraisal_id'] . "' onclick=\"return confirm('Are you sure you want to delete this record?');\">Delete</a>
                              </td>";
                        echo "</tr>";
                    }
                } else {
